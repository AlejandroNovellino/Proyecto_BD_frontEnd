...
export const UpdateCarrera = props => {
	// state
	// state
	...
	console.log(
		`🚀 ~ file: updateCarrera.js:49 ~ UpdateCarrera ~ element`,
		element
	);
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			fetchNumLlamadoCarrera(element.c_fecha);
		};
	...
	const fetchNumLlamadoCarrera = date => {
		const fetchInfo = async date => {
			...
			let aux = allNumLlamada.filter(num => {
				if (element.c_num_llamado === num) return true;

				return !usedNumLlamada.includes(num);
			});
	...
	const handleUpdate = async values => {
		...
		// add the key
		params.c_clave = element.c_clave;
		// try to update
		let response1 = await actions.updateCarrera(params);
		// verify if correct
		if (!response1) {
			console.log(response1);

			console.log("Hubo un error en la actualizacion de la carrera");
			// show the alert
			setAlertShow(true);
		} else {
		...
		<>
			<Container className="mt-5">
				<Alert variant="info">
					...
					<p>
						...
						<br />
						<br />
						Las asignaciones para los puestos que seran premiados deben hacerse
						nuevamente.
					</p>
				</Alert>
			</Container>
			...
			<Modal
				...
				<Modal.Header closeButton>
					<Modal.Title>Carrera actualizada</Modal.Title>
				</Modal.Header>
				<Modal.Body>La carrera se ha actualizado exitosamente</Modal.Body>
			</Modal>
					...
					<Col xs={9}>
						<Card bg={"dark"} text={"white"} className="">
							<Card.Header className="fs-5 fw-bold">
								Carrera a actualizar
							</Card.Header>
							<Card.Body className="px-5">
								<Card.Title className="text-center py-3">
									Ingrese los datos de la carrera a actualizar:
								</Card.Title>
								<Formik
									...
									initialValues={{
										c_nombre: element.c_nombre ? element.c_nombre : "",
										c_fecha: element.c_fecha,
										c_num_llamado: element.c_num_llamado,
										c_pull_dinero_total: element.c_pull_dinero_total
											? element.c_pull_dinero_total
											: "",
										c_distancia: element.c_distancia,
										c_comentario: element.c_comentario
											? element.c_comentario
											: "",
										fk_tipo_carrera: element.fk_tipo_carrera,
										fk_categoria_carrera: element.fk_categoria_carrera,
										fk_pista: element.fk_pista,
									}}>
									{({
										handleSubmit,
										handleChange,
